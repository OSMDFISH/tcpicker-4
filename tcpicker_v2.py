...
def _load_credential_repo() -> Dict:
    ...
    if not CREDENTIAL_FILE.exists():
        ...
        _add_or_update_user(DEFAULT_USERNAME, DEFAULT_PASSWORD, role="admin", force=True)
...
def _add_or_update_user(username: str, password: str, role: str = "user", force: bool = False) -> Tuple[bool, str]:
...
def require_login() -> None:
    ...
    st.caption("Default login: TRIPLE CROWN / RACE#2026!")
    ...
    if submitted:
        ...
        if ok:
            ...
            st.session_state["username"] = username.strip().upper()
...
def render_user_admin_tab():
    ...
    if add_submitted:
        ok, msg = _add_or_update_user(new_username, new_password, role=new_role, force=False)
        if ok:
            st.success(msg)
            st.rerun()
        else:
    ...
    if existing_users:
        ...
        if reset_submitted:
            ...
            repo = _load_credential_repo()
            user_role = repo.get("users", {}).get(reset_user, {}).get("role", user_role)
            ok, msg = _add_or_update_user(reset_user, reset_password, role=user_role, force=True)
            if ok:
                st.success(msg)
                st.rerun()
            else:
